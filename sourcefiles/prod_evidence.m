function [control,control_sign] = prod_evidence( PreState, op_idx, regulators)
...
for j = 1 : size(control_index,1)
    ...
    if con_expr >= 1 
        control(j,1) = con_idx; 
        control_sign(j,1) = 1;                  % regulation sign 1 (activator)
    elseif con_expr == 0 
        control(j,1) = con_idx; 
        control_sign(j,1) = 0;                  % regulation sign 0 (inhibitor)
    end             
end
